...
class OfflineImageStore {
   ...
   */
  getBaseDir = () => {
    return ReactNativeBlobUtil.fs.dirs.CacheDir + '/' + this.store.name;
  };
  ...
  clearStore = (onRestoreCompletion) => {
    ...
    return ReactNativeBlobUtil.fs
      ...
      .then((exists) => {
        ...
        return ReactNativeBlobUtil.fs.unlink(this.getBaseDir());
      })
      .then(() => {
        // On completion
        if (this.store.debugMode) {
  ...
  _getExpiredImages = () => {
    ...
    uriList.forEach((uri) => {
      const createdPlusDaysDate = this._addTime(
        this.entries[uri].createdOn,
        this.store.storeImageTimeout,
      );
  ...
  _removeExpiredImages = (onRestoreCompletion) => {
    ...
    uriListToRemove.forEach((uri) => {
      ...
      const unlinkPromise = ReactNativeBlobUtil.fs
        .unlink(
          this.entries[uri].basePath + '/' + this.entries[uri].localUriPath,
        )
        .then(() => {
  ...
  _updateAsyncStorage = (onRestoreCompletionCallback) => {
    AsyncStorage.setItem(
      `@${this.store.name}:uris`,
      JSON.stringify(this.entries),
      (err) => {
        if (onRestoreCompletionCallback) {
          err
            ? onRestoreCompletionCallback(err)
