...
function IndexChallengeTrainingProgressContainer({ navigation, route }) {
    ...
    GetLocation.getCurrentPosition({
      ...
      .then(async (location) => {
        ...
        if (!isStart) {
          ...
          const trail = challengeData.trails.find(
  ...
  const scanQr = (value) => {
    ...
    GetLocation.getCurrentPosition({
      ...
      .then(async (location) => {
        ...
        if (trailScanned) {
          const isSkipped = !challengeData.trails
            .filter((t) =>
  ...
  const quitTraining = async (skipNavigation = false) => {
    ...
    let data = {
      ...
      status: tempProgress && tempProgress.length ? 'complete' : 'incomplete',
      total_time: Math.abs(
        moment
  ...
  const getNextTrails = (trailIndex) => {
    if (trailIndex === 1) {
      return [trailIndex + 1];
    } else if (trailIndex === challengeData.trails.length) {
      return [trailIndex - 1];
    } else {
        ...
        >
          ...
          {!!toastModalVisible && (
            <CheckpointScanToastComponent
              ...
              nextCheckpoint={challengeData.trails.filter((t) =>
                currentTrailIndex.length > 1
                  ? t.trail_index === currentTrailIndex[0] ||
                    t.trail_index === currentTrailIndex[1]
                  : t.trail_index === currentTrailIndex[0],
          ...
          <ConfirmationPopupComponent
            ...
            message="You will be ending the session now. This cannot be undone. Confirm?"
...
export default IndexChallengeTrainingProgressContainer;
